...
from hiyori import HttpRequestMethod, HttpVersion, options


class DeleteEchoProtocol(helpers.BaseMockProtocol):
...
async def test_simple(mocked_server: helpers.MockedServer) -> None:
    mocked_server.mock_proto_cls = DeleteEchoProtocol

    response = await options(f"http://localhost:{mocked_server.port}")
    ...
    assert response.request.method == HttpRequestMethod.OPTIONS
