...
from hiyori import HttpRequestMethod, HttpVersion, delete
...
async def test_simple(mocked_server: helpers.MockedServer) -> None:
    ...
    response = await delete(f"http://localhost:{mocked_server.port}")
    ...
    assert response.request.method == HttpRequestMethod.DELETE
