...
from hiyori import HttpRequestMethod, HttpVersion, put


class PutEchoProtocol(helpers.BaseMockProtocol):
...
async def test_simple(mocked_server: helpers.MockedServer) -> None:
    mocked_server.mock_proto_cls = PutEchoProtocol

    response = await put(
        f"http://localhost:{mocked_server.port}", json={"a": "b", "c": [1, 2]}
    )
    ...
    assert response.request.method == HttpRequestMethod.PUT
